...
const a = new ControlService(new ControlStore({ remote: db })), b = new ControlService(new ControlStore({ remote: db }));
const key = `smoke-${randomUUID()}`, project = projectId(key);
try {
  const attempts = await Promise.allSettled(Array.from({ length: 6 }, (_, i) => (i % 2 ? a : b).reserve(key, { provider: 'cdp', maxConcurrent: 1 })));
  ...
  assert.equal((await b.store.events({ seqs: [delivery.eventSeq] }))[0].type, 'project.settings.updated', 'delivery created with its event');
  ...
  console.log('Supabase smoke passed: RPC permissions, shared admission, command gate vs takeover, credentials, event fan-out, cursor.');
} finally {
  ...
  const left = (await a.store.load([...kinds.map(kind => ({ kind, project })), { kind: 'project', id: project }])).flat().length + (await a.store.events({ project })).length;
