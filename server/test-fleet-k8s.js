...
const { control, hash } = await import('./src/control/service.js');
const { createManaged, removeManaged, managedConfigured, verifyRuntime } = await import('./src/control/managed.js');
const k8s = await import('./src/control/fleet/k8s.js');
...
try {
  ...
  await check('no credential appears in any kubectl argv', async () => {
    for (const c of [applied]) {
      assert.ok(!c.args.some((a) => a.includes(secret.stringData.OYA_ENROLLMENT_TOKEN)), 'enrollment token not in argv');
  ...
  await check('privilege escalation disallowed', () => assert.equal(container.securityContext.allowPrivilegeEscalation, false));
  await check('seccomp is the runtime default', () => assert.equal(container.securityContext.seccompProfile.type, 'RuntimeDefault'));
  await check('no Kubernetes API token is mounted', () => assert.equal(pod.spec.automountServiceAccountToken, false));
  await check('a spent enrollment token cannot be reused by a restart', () => assert.equal(pod.spec.restartPolicy, 'Never'));
  ...
  await check('the image the pod runs is the digest-pinned one', () => assert.equal(container.image, process.env.OYA_MANAGED_IMAGE));
  ...
  passed++; console.log('  ✅ a mutable image tag is refused; only a digest is accepted');
  ...
  passed++; console.log('  ✅ a missing NetworkPolicy is refused');

  process.env.OYA_FAKE_NP = JSON.stringify({ spec: { podSelector: { matchLabels: { app: 'something-else' } }, policyTypes: ['Egress'] } });
  await assert.rejects(verifyRuntime(), { code: 'unsafe_network' }, 'a policy selecting other pods is refused');
  passed++; console.log('  ✅ a NetworkPolicy that does not select these pods is refused');
  ...
  passed++; console.log('  ✅ a NetworkPolicy that does not restrict Egress is refused');
  ...
  await assert.rejects(removeManaged(podName, KEY, session.id, 'cluster-uid-1', 'k8s'),
    { code: 'ownership_mismatch' }, 'another tenant\'s pod is not deleted');
  passed++; console.log('  ✅ cleanup refuses a pod owned by another key');

  process.env.OYA_FAKE_POD = JSON.stringify({
    metadata: { annotations: { 'ai.getoya.owner': hash(KEY), 'ai.getoya.session': 'a-different-session' } },
  });
  await assert.rejects(removeManaged(podName, KEY, session.id, 'cluster-uid-1', 'k8s'),
    { code: 'ownership_mismatch' }, 'a pod from another session is not deleted');
  passed++; console.log('  ✅ cleanup refuses a pod belonging to another session');
  ...
  await assert.rejects(removeManaged(podName, KEY, session.id, 'cluster-uid-1', 'k8s'),
    { code: 'runtime_unavailable' }, 'a pod in the wrong cluster is not deleted');
  passed++; console.log('  ✅ cleanup refuses a different cluster than the one that created the pod');
  ...
  await check('a docker-created session cleans up via docker after a switch to k8s', () => {
    ...
    assert.ok(routed.every((c) => c.tool === 'docker'), `expected docker, saw ${routed.map((c) => c.tool).join(',')}`);
  });
