...
import { acquire } from './src/providers.js';

const originalFetch = globalThis.fetch;
const cases = [
  ['anchor', { data: { id: 'test-id', cdp_url: 'wss://example.com/cdp' } }, '/v1/sessions', 'DELETE', '/test-id', undefined],
  ['browserbase', { id: 'test-id', connectUrl: 'wss://example.com/cdp' }, '/v1/sessions', 'POST', '/test-id', { status: 'REQUEST_RELEASE' }],
  ['browseruse', { id: 'test-id', cdpUrl: 'wss://example.com/cdp' }, '/api/v2/browsers', 'PATCH', '/test-id', { action: 'stop' }],
  ['steel', { id: 'test-id', websocketUrl: 'wss://connect.steel.dev?sessionId=test-id' }, '/v1/sessions', 'POST', '/test-id/release', undefined],
];
try {
  ...
  calls = 0;
  globalThis.fetch = async () => ++calls === 1 ? Response.json({ id: 'id', connectUrl: 'wss://example.com' }) : Response.json({}, { status: calls === 2 ? 500 : 200 });
  ...
  await assert.rejects(acquire({ provider: 'steel', env: { STEEL_API_KEY: 'secret' } }), e => !e.message.includes('secret'));
