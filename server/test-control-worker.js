...
Object.assign(process.env, { OYA_DATA_DIR: directory, OYA_PROFILE_SECRET: 'worker-contract', API_KEYS: 'worker-owner' });
delete process.env.SUPABASE_URL; delete process.env.SUPABASE_SERVICE_KEY;
const { control, hash, projectId } = await import('./src/control/service.js');
const { tick, deliver, maintainControl, stopWorkers } = await import('./src/control/worker.js');
const { ownerFor } = await import('./src/control/cluster.js');
const { createEgressServer } = await import('./src/control/egress.js');
const service = control();
const patch = (kind, id, changes) => service.store.transact(async tx => { Object.assign(await tx.get(kind, id), changes); });
try {
  ...
  const delivery = async (url, body, headers) => {
    ...
    assert.equal(signature, `v1=${createHmac('sha256', hook.secret).update(`${time.slice(2)}.${body}`).digest('hex')}`);
  ...
  assert.equal(event.state, 'delivered'); assert.equal(calls, 2, 'delivery lease excludes concurrent workers');

  const attached = await service.reserve('r1', { provider: 'cdp' });
  assert.equal((await service.complete('r1', attached.id, 502, { error: 'unreachable' })).state, 'failed', 'a failed attach frees its slot');
  ...
  await assert.rejects(service.adopt('r2', { id: desktop.id, provider: 'oya-desktop', maxConcurrent: 1 }), { code: 'quota_exceeded' }, 'reconnect rechecks capacity');
  const enrolling = await service.reserve('r3', { provider: 'oya-selfhosted' });
  await patch('session', enrolling.id, { instance: 'provisioner' });
  assert.equal((await service.adopt('r3', { id: enrolling.id, provider: 'oya-selfhosted' })).state, 'ready', 'enrollment may land on any replica');
  ...
  assert.equal((await service.cancel('r4', legacy.id, { force: true })).state, 'stopped', 'force reconciles an undeletable resource');
  ...
  const waiting = await service.reserve('r5', { provider: 'oya-selfhosted', managed: true, request: { queueMs: 60000 } });
  ...
  assert.equal(budgeted.find(x => x.id === waiting.id).state, 'queued', 'budget exhaustion leaves queued work to its deadline');
  ...
  await assert.rejects(service.reserve('r6', { provider: 'oya-selfhosted', managed: true }), { code: 'budget_admission' }, 'pruned spend still counts');
  ...
  await service.store.transact(async tx => { await tx.get('project', projectId('r6')); });
  ...
  await service.update('r7', governed.id, { state: 'ready', managed: true, egressHash: hash('t'), policies: [{ humanHosts: ['example.com'] }] });
  const proxy = createEgressServer(); await new Promise(r => proxy.listen(0, '127.0.0.1', r));
  const status = await new Promise((resolve, reject) => {
    const req = http.request({ host: '127.0.0.1', port: proxy.address().port, method: 'CONNECT', path: 'example.com:443', headers: { 'Proxy-Authorization': `Basic ${Buffer.from(`${governed.id}:t`).toString('base64')}` } });
    req.on('connect', res => { res.socket.destroy(); resolve(res.statusCode); }); req.on('error', reject); req.end();
  });
  proxy.close();
  assert.equal(status, 403, 'egress enforces humanHosts while the agent has control');
  console.log('Worker contracts passed: cleanup retry and release, signed webhook retry, concurrent delivery exclusion, attach failures, dead-owner routing, reconnect capacity, cross-replica enrollment, reconciliation, budget vs queue, metered spend survives pruning, no-op commits, egress humanHosts.');
} finally {
  await stopWorkers(); await service.store.close(); await rm(directory, { recursive: true, force: true });
}
process.exit(0);
