...
const { validateApiKey } = await import('./src/auth.js');
...
try {
  ...
  assert(unique.size === 3, `Commands hit 3 different browsers (got ${unique.size}: ${browsers.map(b => b.slice(0,8)).join(', ')})`);
  ...
  b4.ws.send(JSON.stringify({
    type: 'cookie_dump',
    cookies: [
      { name: 'token', value: 'xyz789', domain: '.test.com', path: '/', secure: false, httpOnly: false, sameSite: 'none' },
    ],
  }));
  await wait(500);
