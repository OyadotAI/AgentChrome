...
let stored = [];
const columns = new Set(['id', 'owner', 'name', 'seed', 'prefs', 'proxy', 'max_concurrent', 'is_default', 'created_at', 'last_used_at', 'updated_at']);
globalThis.fetch = async (url, init) => {
  assert.equal(new URL(url).pathname, '/rest/v1/personas');
  if (init.method === 'POST') {
    stored = JSON.parse(init.body);
    for (const row of stored) for (const key of Object.keys(row)) assert.ok(columns.has(key), `Unknown database column: ${key}`);
...
const custom = personas.create(key, { name: 'Stable device', maxConcurrent: 3, prefs: { platform: 'MacIntel', timezone: 'Pacific/Honolulu' } });
...
console.log('Profile database round-trip passed: identity, ownership, capacity, timestamps and device preferences preserved.');
