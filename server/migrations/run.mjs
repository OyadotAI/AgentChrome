...
async function main() {
  ...
  for (const name of readdirSync(HERE).filter((f) => f.endsWith('.sql')).sort()) {
  ...
  await psql(['-c', 'create table if not exists public.schema_migrations (filename text primary key, applied_at timestamptz not null default now())']);
  ...
  for (const [name, path] of todo) {
    ...
    await psql(['--single-transaction', '-f', path, '-c',
