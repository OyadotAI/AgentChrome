...
const mfa = await import('./src/mfa.js');

const CHROME = [
  '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',
  '/usr/bin/google-chrome', '/usr/bin/chromium', '/usr/bin/chromium-browser',
].find((p) => existsSync(p));
if (!CHROME) { console.log('⏭  No Chrome binary — skipping challenge test'); process.exit(0); }
...
try {
  ...
  assert(unconfigured.solved === false && unconfigured.method === 'none',
  ...
  assert(native.method === 'provider' && !native.solved,
    'a provider that solves natively is not solved over the top');
  ...
  assert(await evaluate(`document.querySelector('[name="cf-turnstile-response"]').value`) === 'test-token-123',
