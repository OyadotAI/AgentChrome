...
const { handleJsonList, handleUpgrade } = await import('./src/gateway.js');
const app = express(); app.use(express.json()); app.use('/api', router); app.get('/json/list', handleJsonList);
...
async function call(path, method = 'GET', body, key = 'owner') {
  const r = await fetch(base + path, { method, headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
...
try {
  ...
  registry.add(id, { apiKey: 'owner', name: 'Test', driver: { wsUrl: 'ws://127.0.0.1:9', send: async () => ({ ok: true }), close() {} }, provider: 'cdp', clientType: 'cdp' });
  ...
  assert.equal((await call(`/api/browsers/${id}`, 'GET', undefined, viewer.token)).body.cdpUrl, undefined, 'viewers cannot obtain full-access URLs');
  ...
  const { authenticateToken } = await import('./src/auth.js');
  ...
  assert.equal((await call(`/api/browsers/${id}/command`, 'POST', { action: 'navigate', params: { url: 'https://example.com' } })).status, 409);
  assert.equal((await call(`/api/control/sessions/${id}/input`, 'POST', { action: 'click', params: { element_id: 1 } })).status, 200);
  assert.equal((await call(`/api/control/sessions/${id}/input`, 'POST', { action: 'keyboard_type', params: { text: 'hello' } })).status, 200);
  ...
  assert.equal((await call('/api/control/events')).body.events.some(e => e.type === 'session.stopped'), true);
  ...
  registry.add(quiet, { apiKey: 'owner', name: 'Quiet', driver: { wsUrl: `ws://127.0.0.1:${silent.address().port}`, send: async () => ({ ok: true }), close() {} }, provider: 'cdp', clientType: 'cdp' });
  const cdp = new WebSocket(`${base.replace('http:', 'ws:')}/connect?browser=${quiet}`, { headers: { Authorization: 'Bearer owner' } });
  await new Promise((resolve, reject) => { cdp.once('open', resolve); cdp.once('error', reject); });
  cdp.send(JSON.stringify({ id: 1, method: 'Runtime.evaluate', params: { expression: 'new Promise(() => {})', awaitPromise: true } }));
  await new Promise(r => setTimeout(r, 100));
  assert.equal((await control().session('owner', quiet)).inFlight, 1, 'the command holds an in-flight slot');
  assert.equal((await call(`/api/control/sessions/${quiet}/control`, 'POST', { action: 'acquire' })).status, 200, 'takeover waits out an unanswered command');
  cdp.terminate(); silent.close();
  ...
  assert.equal((await call('/api/control', 'GET', undefined, enrolled.token)).status, 403, 'managed-browser credentials cannot use the API');
  const { authenticateToken: authenticate } = await import('./src/auth.js');
  assert.equal((await authenticate(enrolled.token, { allowBrowser: true })).sessionId, quiet);
  assert.equal((await control().read('owner')).credentials.some(c => c.label === 'Managed browser'), false, 'managed-browser credentials are not listed');
  await control().update('owner', quiet, { state: 'stopped' });
  await assert.rejects(authenticate(enrolled.token, { allowBrowser: true }), { status: 401 }, 'the credential ends with its session');
  console.log('Control API passed: discovery privacy, role boundaries, credential/ticket revocation, takeover, all-path drain, queued creation and cancellation, stuck-command takeover, managed-browser credential scope.');
} finally {
  server.closeAllConnections(); await new Promise(resolve => server.close(resolve));
  await control().store.close(); await rm(directory, { recursive: true, force: true });
}
process.exit(0);
