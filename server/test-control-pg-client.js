...
const { pgRemote, closePgPool } = await import('./src/control/pg-client.js');
...
const same = await Promise.all([a, b].map((c) => c.reserve(key, {
  provider: 'cdp', idempotencyKey: 'shim-same', request: { name: "quoted ' safely" },
})));
assert.equal(same[0].id, same[1].id, 'idempotency key is honoured through the shim');

await closePgPool();
console.log('pg shim contracts passed: pooling, jsonb arrays, named + null args, admission race, gating, credentials, idempotency.');
