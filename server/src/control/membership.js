...
projectAccountRouter.get('/', wrap(async (req, res) => {
  ...
  const [ownedRows, joined] = await control().store.load([{ kind: 'project', states: [req.user.id] }, { kind: 'membership', states: [req.user.id] }]);
  ...
  const names = new Map((await control().store.load(memberships.map(m => ({ kind: 'project', id: m.project })))).flat().filter(r => !r.body.deletedAt).map(r => [r.id, r.body.name]));
  res.json([
    ...owned.map(r => ({ id: r.id, name: AUTO_NAME.test(r.body.name) ? labels.get(r.id) || r.body.name : r.body.name, role: 'administrator', owner: true })),
...
projectAccountRouter.post('/join', wrap(async (req, res) => {
  ...
  const membership = await control().store.transact(async tx => {
    ...
    if ((await tx.get('project', invite.project))?.deletedAt) throw fault('not_found', 'Project not found', 404);
...
projectAccountRouter.post('/:id/access', wrap(async (req, res) => {
  ...
  if (!p || p.body.deletedAt || !role) throw fault('not_found', 'Project not found', 404);
  const key = control().projectKey(p.body);
  res.json(await control().credential(key, { role, label: 'Console access', expiresAt: Date.now() + 3600000 }, req.user.id));
}));
projectAccountRouter.patch('/:id', wrap(async (req, res) => {
...
projectAccountRouter.post('/:id/key', wrap(async (req, res) => {
  ...
  if (!p || p.deletedAt || p.ownerUser !== req.user.id) throw fault('not_found', 'Project not found', 404);
