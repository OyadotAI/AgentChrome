...
const [{ default: express }, { WebSocketServer }, { router }, { handleConnection }, gateway, { registry }, profiles, logins, mfa, { Oya }] = await Promise.all([
  import('express'), import('ws'), import('./src/api.js'), import('./src/ws-handler.js'), import('./src/gateway.js'),
  import('./src/connection-registry.js'), import('./src/personas.js'), import('./src/cookie-store.js'), import('./src/mfa.js'), import('../packages/sdk/dist/index.js'),
]);
const app = express();
app.use(express.json()); app.use('/api', router);
app.get('/fixture', (req, res) => res.type('html').send(`<!doctype html><title>loading</title>
...
async function chrome() {
  const child = spawn(chromePath, ['--headless=new', '--remote-debugging-port=0', '--no-first-run', '--no-default-browser-check', `--user-data-dir=${join(scratch, `chrome-${children.length}`)}`, 'about:blank'], { stdio: ['ignore', 'ignore', 'pipe'] });
  children.push(child);
...
try {
  ...
  logins.mergeDump(profile.id, [{ name: 'account', value: 'alice', domain: '127.0.0.1', path: '/', httpOnly: true, hostOnly: true }]);
  ...
  const reloaded = await import(`./src/cookie-store.js?reload=${Date.now()}`);
  ...
  mfa.reset(); mfa.restore(); assert.equal(mfa.describe(profile.id).type, 'totp');
  ...
  assert.equal(await registry.get(browser.id).driver.evaluateMain("document.querySelector('input').value"), 'replacement');
  ...
  await registry.get(browser.id).driver.evaluateMain("localStorage.clear(); document.cookie='newSession=renewed; path=/'");
  ...
  assert((await registry.get(next.id).driver.conn.send('Network.getAllCookies', {}, registry.get(next.id).driver.sessionId)).cookies.some((c) => c.name === 'newSession'));
  await next.stop();
