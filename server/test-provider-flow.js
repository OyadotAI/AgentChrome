...
for (const key of ['STEEL_API_KEY', 'ANCHOR_API_KEY', 'BROWSERBASE_API_KEY', 'BROWSERUSE_API_KEY', 'OYA_PROVIDERS']) delete process.env[key];
const { router } = await import('./src/api.js');
const { handleUpgrade, sessions } = await import('./src/gateway.js');
const { registry } = await import('./src/connection-registry.js');
const { pool, ProviderPool } = await import('./src/routing.js');
...
server.on('upgrade', (req, socket, head) => {
  if (req.url === '/upstream') upstreamServer.handleUpgrade(req, socket, head, ws => upstreamServer.emit('connection', ws, req));
  else handleUpgrade(req, socket, head).catch(() => socket.destroy());
});
const realFetch = globalThis.fetch;
const request = async (path, body, method = 'POST', owner = 'owner-a') => {
  const res = await realFetch(origin + path, { method, headers: { Authorization: `Bearer ${owner}`, 'Content-Type': 'application/json' }, ...(body ? { body: JSON.stringify(body) } : {}) });
  return { status: res.status, body: await res.json() };
};
try {
  for (const cfg of [
    { name: ' ' }, { name: 'bad/name' }, { name: 'no-url', type: 'cdp' },
    { name: 'unknown', type: 'made-up' }, { name: 'zero', type: 'steel', maxConcurrent: 0 },
    { name: 'negative', type: 'steel', weight: -1 }, { name: 'nan', type: 'steel', priority: 'abc' },
  ...
  const added = await request('/gateway/providers', { name: 'my steel', type: 'steel', apiKey: 'credential-a', priority: 0, active: 999, cooldownUntil: 9999999999999 });
  ...
  await request('/gateway/providers', { name: 'direct', type: 'cdp', wsUrl: 'ws://127.0.0.1:9222/cdp?token=private-route-secret' });
  await config.drain();
  ...
  config.reset(); await config.restore();
  const restored = new ProviderPool(); config.restoreRouting(restored);
  ...
  let failHandshake = false, releases = 0;
  globalThis.fetch = async (url, opts) => {
    if (String(url).startsWith('https://api.steel.dev')) {
      assert.equal(opts.headers['steel-api-key'], 'credential-a');
      if (String(url).endsWith('/release')) { releases++; return Response.json({}); }
      return Response.json({ id: 'gateway-vendor', websocketUrl: failHandshake ? 'ws://127.0.0.1:1/fail' : upstreamUrl });
    }
    return realFetch(url, opts);
  };
  ...
  server.on('upgrade', (req, socket, head) => {
    if (new URL(req.url, origin).pathname === '/upstream') upstreamServer.handleUpgrade(req, socket, head, ws => upstreamServer.emit('connection', ws, req));
    else handleUpgrade(req, socket, head).catch(() => socket.destroy());
  });
  ...
  await [...sessions.values()][0].destroy('test'); fallback.close();
  ...
  registry.add('stop-failure', { apiKey: 'owner-a', clientType: 'cdp', provider: 'steel', driver: { close() {} }, release: async () => { throw new Error('release failed'); } });
  ...
  await config.drain(); config.reset(); await config.restore();
  const afterDelete = new ProviderPool(); config.restoreRouting(afterDelete);
