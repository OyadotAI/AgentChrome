...
if (!CHROME) { console.log('⏭  No Chrome binary — skipping MCP lifecycle test'); process.exit(0); }

let passed = 0, failed = 0;
const assert = (ok, msg) => { if (ok) { passed++; console.log(`  ✅ ${msg}`); } else { failed++; console.log(`  ❌ ${msg}`); } };
const freePort = () => new Promise((resolve) => {
  const s = createNetServer().listen(0, '127.0.0.1', () => { const { port } = s.address(); s.close(() => resolve(port)); });
});
...
const server = spawn(process.execPath, ['src/index.js'], {
  ...
  env: {
    ...process.env, PORT: String(port), API_KEYS: KEY, OYA_DATA_DIR: join(dir, 'data'),
    OYA_PROFILE_SECRET: 'c'.repeat(64), OYA_ALLOW_PRIVATE_TARGETS: 'true', OYA_UI_MODE: '',
    SUPABASE_URL: '', SUPABASE_SERVICE_KEY: '', DAYTONA_API_KEY: '', OYA_API_KEY: '',
  },
...
const api = (path, body) => fetch(`${base}/api${path}`, {
...
try {
  ...
  await client.connect(new StreamableHTTPClientTransport(new URL(`${base}/mcp/pool`),
  ...
  assert(tools.includes('start_browser') && tools.includes('stop_browser'), 'start_browser and stop_browser are listed');
  ...
  assert(!page.error && /Hello agent/.test(page.text) && /Press me/.test(page.text), 'it navigated, and analyze_page reads the page');
