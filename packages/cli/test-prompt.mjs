...
const { choose, ask, InputError } = await import('./src/prompt.ts');
...
function captureStdout() {
  ...
  stdout.write = (chunk, ...rest) => { chunks.push(String(chunk)); return real('', ...rest.filter(() => false)) || true; };
  return { text: () => chunks.join(''), restore: () => { stdout.write = real; } };
}
...
function pipedRun(script, input) {
  const child = spawnSync(process.execPath, ['--input-type=module', '-e', script], {
    input, encoding: 'utf8', cwd: new URL('.', import.meta.url).pathname,
  });
