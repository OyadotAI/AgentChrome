...
//   node test-cdp-front-door.mjs http://127.0.0.1:9222 <userData>/profiles/<id>.json
import { chromium } from 'playwright-core';
import assert from 'node:assert/strict';
import fs from 'node:fs';

const [base = 'http://127.0.0.1:9222', profilePath] = process.argv.slice(2);
if (!profilePath) { console.error('usage: node test-cdp-front-door.mjs <cdp base url> <profile json>'); process.exit(1); }
const profile = JSON.parse(fs.readFileSync(profilePath, 'utf8'));

const version = await (await fetch(`${base}/json/version`)).json();
assert.match(version.webSocketDebuggerUrl, new RegExp(`^ws://${new URL(base).host}/devtools/browser/`), 'ws URL must point at the front door');
...
const got = await page.evaluate(() => ({
  ...
  viewport: [innerWidth, innerHeight], tz: Intl.DateTimeFormat().resolvedOptions().timeZone,
